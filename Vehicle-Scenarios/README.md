...
2. Add the **parent directory** of the cloned repository to the **Extension Search Paths** inside the Extensions window.
    - *(Navigate to: Extensions > Gear Icon (Settings) > Extension Search Paths)*
3. Verify that **"SIX VEHICLE SCENARIOS"** appears under **Extensions > THIRD PARTY > User**.

## Vehicle Scenario Test Setup

1. Open **Extensions**, search for `actor`, and enable **ACTOR SDG UI**.
2. In the **ACTOR SDG UI**, set the **Config File Path**:
...
## Vehicle Scenario Test Using Custom Extension

1. **Select a Scenario (1-5)**.
    - *Note: Selecting "First-person" switches the view to the camera mounted on the front of the vehicle.*
